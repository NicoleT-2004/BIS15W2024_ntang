...
ui <- fluidPage(
   ...
   selectInput("y", "Select Y Variable", choices = c("bill_length_mm", "bill_depth_mm", "flipper_length_mm", "body_mass_g"), selected = "bill_length_mm")
    ...
    plotOutput("plot", width="500px", lenght="400px")
)
...
ui <- fluidPage(
  ...
  selectInput("x", "Select Fill Type", choices = c("trophic.guild", "thermoregulation"), 
...
server <- function(input, output, session) {
  ...
  output$plot <- renderPlot({
    ...
    ggplot(data=homerange, aes_string(x="locomotion",fill=input$x))+
      geom_bar(position = "dodge", color="aliceblue")+
      labs(x=NULL, fill="Fill Varible")
  })
