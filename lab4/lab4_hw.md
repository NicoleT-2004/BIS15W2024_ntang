...
filter(homerange, mean.mass.g == min(owls$mean.mass.g))
```
...
```
The smallest owl is [Eurasian pgymy owl](https://en.wikipedia.org/wiki/Eurasian_pygmy_owl).
...
## #   PPMR <dbl>, prey.size.reference <chr>
```
[Caracara](https://www.allaboutbirds.org/guide/Crested_Caracara/id) has the largest homerange.
