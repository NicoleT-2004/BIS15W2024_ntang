...
```
NA is represented as -999, -999.000
...
```r
amniota <- read_csv("data/amniota.csv", na=c("NA", "-999.000", "-999")) 
```
...
```
Yes. Mammalia have no eggs thus the pct_miss is 100%. NA not only means lacking a data but cana also means the catagory is not applicable.
...
## 4 Ter        1104     16.3
```
The precent of NAs means the preent of animals which did not fit in this catagory.
