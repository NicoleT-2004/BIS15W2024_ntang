...
```{r}
topspeed <- c(chiron, venom_GT, veyron)
mean(topspeed)
```
...
It means that R cannot find the mean yet because a data is missing. NAs will stop R from interpreating the data
...
```
When doing an experiment, it allows us to get temporary results and outcomes from incomplete data without changing the whole data set.
