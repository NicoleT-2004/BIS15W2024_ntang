...
fish %>% #work with the fish data
  select(lakeid,radii_length_mm) %>% #pull out variable interest
  filter(lakeid == "AL" | lakeid == "AR") %>% #only these lakes
  filter(between(radii_length_mm , 2 , 4)) %>% #between 2 and 4
  arrange(desc(radii_length_mm)) #sort to make it easier to read
```
...
```{r}
fish %>% #fish data
  select("radii_length_mm", "fish_id", "scalelength") %>% #needed data
  filter(fish_id == 300) %>% #fish id is 300
  arrange(radii_length_mm)#sort by radii length
```
